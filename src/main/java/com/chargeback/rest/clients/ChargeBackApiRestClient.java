...
public interface ChargeBackApiRestClient {
	...
	public ResponseEntity<String> getFreeResourceValue(@PathVariable(value="resourceType") String resourceType);
	
	@RequestMapping(value="/metrics/getSpaceList{orgName}", method=RequestMethod.GET, produces=MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<String>> getSpaceList(@PathVariable(value="orgName") String orgName);
